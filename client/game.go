...
func playGame(connection *sloopNet.GameConn, first bool) error {

	// tell the opponent we are ready to play the game
	err := connection.SendMsg("_g_begin:")
	if err != nil {
		return err
	}

	// wait for opponent to send a "start game" message
	err = connection.ReadMsg()
	if err != nil {
		return err
	}
	...
	// wait for the opponent to agree to turn order
	err = connection.ReadMsg()
	if err != nil {
		fmt.Println(err)
